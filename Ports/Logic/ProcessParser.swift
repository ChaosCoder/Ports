...
protocol ProcessParserType {
    func parse(output: String) throws -> [Process]
}

struct ProcessParser: ProcessParserType {
    func parse(output: String) throws -> [Process] {
        ...
        for line in output.components(separatedBy: "\n") {
            ...
            try parse(prefix: prefix, value: value, processes: &processes, process: &process, socket: &socket)
        }
    ...
    private func parse(prefix: Character, value: String, processes: inout [Process], process: inout PartialProcess?, socket: inout PartialSocket?) throws {
        switch prefix {
        case "p":
            if let finishedSocket = socket {
                process?.sockets.append(finishedSocket)
